...
class HexGame(tk.Tk):
    ...
    def show_game(self, difficulty):
        """play the game with the difficulty selected
            difficulty is defined in constants
        """
    ...
    def show_enter_high_score_screen(self,score):
        """if player gets a high score show this screen. This screen will only ever be entered from PlayGame,
...
class MainGameLayout(tk.Frame):
    """This contains the layout for the PlayGame screen. It separates the logic from the layout."""
    """ It does not add in bindings etc to the entries, as this is done and handled in the game logic"""
    ...
    def create_target_box(self):
        ...
        self.targetBox = self.colour_canvas.create_rectangle(x1, y1, x2, y2, fill="#888888", outline="", width=0)
    ...
    def create_guess_box(self):
        ...
        x2 = x1 + COLOUR_BOX_WIDTH
        y2 = y1 + COLOUR_BOX_HEIGHT
        self.guessBox = self.colour_canvas.create_rectangle(x1, y1, x2, y2, fill="#888888", outline="", width=0)     
    ...
    def create_guess_grid(self):
        ...
        #guessLineStructure = [] - This is just one line of the grid (here for clarity)
...
class MainGameLogic(tk.Frame):
    """
    This controls the game, makes the checks and evaluated the entries, and updates the layout via the layout passed to it.
    It adds a binding to each entry box to monitor for key presses and responds to each accordingly. This way it can auto advance
    ...
    def validate_single_char(self, input_text):
        """validation function to ensure only one character is entered
            and that character is a valif hex digit, ie 0-9, a-f
    ...
    def advance_focus(self, event, current_idx):
        """Moves the cursor to the next entry box and selects it (so it will overwrite the value already there if the
            player enters a different value)
            event is the ky pressed event
            current_idx is an integer value of wheich entry box we are currently in
        """
        ...
        # Check if the key is one we are monitoring for elsewhere. If it is, do nothing, to stop double handling
        ...
        if current_text and len(current_text) == 1 and current_idx < len(self.layout.entry_boxes) - 1:
    ...
    def regress_focus(self, event, current_idx):
        """Moves the cursor to the previous box (if it exists) and selects it (so it will overwrite the value already there if the
            ...
            current_idx is an integer value of which entry box we are currently in
        """
        # If Backspace or Delete was pressed and the entry is empty (or about to become empty)
        if not self.layout.entry_input_values[current_idx].get() and current_idx > 0:
    ...
    def submit_entry(self, event, current_idx):
        ...
        #check if entries are already disabled, ie the game is over (should not get here!)
        if self.layout.entry_boxes[0].cget("state") == "disabled":
            return
        #Validate entries are all full, if not ignore the enter key
        if (not self.validate_entries()):
            return
        else:
            #Since the keys are validated at each key press we don't need to validate them here as well
            #adding guess number
            self.guess_count += 1
            ...
            if (self.difficulty == DIFFICULTY_STANDARD):
                ...
                error_margin_green = self.error_margin_indicator(difference_green)
                error_margin_blue = self.error_margin_indicator(difference_blue)
            ...
            game_over = False
            if difference_red == 0 and difference_green == 0 and difference_blue == 0:
                #Player won the game
                #Did they get a high score?
                if (self.accuracy > self.lowest_score_for_difficulty):
                    self.master.master.show_enter_high_score_screen(self.accuracy)
                    return
    ...
    def update_guess_grid(self, entry_boxes, error_margin_red, error_margin_green, error_margin_blue):
        """Adds the submitted guess to the previous guesses but adding it in the first position, 
            thus moving every previous guess down
            entry_boxes is a list of references to each guess entry box
            error_margin_red etc is an indicator for how far out the guess was (integer)
        """
        # guess_line_values contains the error margin for each hex colour pair, then each hex digit the player submitted as a guess
    ...
    def error_margin_indicator(self, number):
        """Calculates the "error" in the player's guess to give a hint as to how far out their guess is
            We do this my taking the log of the absolte difference between the guessed value and the target value.
            This can be adjusted by changeing the base log value, contained in ERROR_MARGIN_INDICATOR_DIFFICULTY,
            ie log 2 is easiest , log 3+ will give a wider range. 
            ERROR_MARGIN_INDICATOR_DIFFICULTY is the base log value. It should be set as an integer from 2+
            number is an integer, indicating error
        """
        # Calculates the logarithm of the absolute of a number and rounds it up to the next integer.
        # If the original number is negative it then returns this value as a negative. 
        # Need to add 1, since if you are out by 1 then log will return zero, but the guess is still not quite correct
        if number == 0:
            return 0
        elif number < 0:
    ...
    def update_score(self, error_margin_red, error_margin_green, error_margin_blue):
        """The score is not displayed, but is used to determin the high score charts.
            ...
            error_margin_xxx is an integer value for actual difference between target and guess
        """
    ...
    def get_lowest_score_for_difficulty(self):
        ...
        #If the high score file has an error (eg not found etc) then the lowest score is zero
        ...
        return high_score_data[self.difficulty][len(high_score_data[self.difficulty])-1]["score"]


class PlayGame(tk.Frame):
    """The actual game!
        We use different classes for the layout and logic, hence we need this class to bring them together.   
    """
    def __init__(self, master):
        super().__init__(master, bg="white")
        self.master = master
...
class HelpScreen(tk.Frame):
    """Display text file containing the help
        Due to limitations with TKinter we only display very simple text within the game window. 
        We display more complex help text as html in the browser (since it's not possible using TKinter)
    """
    def __init__(self, master):
        super().__init__(master, bg="white")
...
class EnterHighScoreScreen(tk.Frame):
    """Enter a new high score
        This screen is ONLY entered from PlayGame, the player CANNOT navigate here
    """
    ...
    def read_scores(self,high_score_filename):
        ...
        """
        #We use try, because we're reading from an extrnal file and a lot can go wrong
        try:
            with open(high_score_filename, 'r') as high_score_file:
                return json.load(high_score_file)
        except FileNotFoundError:
            # If the file doesn't exist, return a default structure.
            print(f"File '{high_score_filename}' not found. Creating a new score structure.")
            return {
                DIFFICULTY_STANDARD: [],
                DIFFICULTY_EXPERT: []
            }
        except json.JSONDecodeError:
            print(f"Error decoding JSON from '{high_score_filename}'. File may be empty or corrupt.")
            return {
                DIFFICULTY_STANDARD: [],
                DIFFICULTY_EXPERT: []
            }
    ...
    def update_scores(self):
        ...
        else:  #The difficult exists so we append out score to that difficulty
